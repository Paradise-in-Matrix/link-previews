...
(defoverride ^:defer link-preview-card [url hs-url room-id]
  (let [yt-id       (extract-youtube-id url)
        is-short?   (and yt-id (clojure.string/includes? url "/shorts/"))
        preview     @(re-frame/subscribe [:media/url-preview url])]

    (if-not preview
      (do
        (if (re-find plugin-regex url)
          (re-frame/dispatch [:media/fetch-plugin-preview url room-id])
          (re-frame/dispatch [:media/fetch-url-preview url room-id]))
        [:div.link-preview-container.is-loading
         [:div.preview-skeleton]])

      (let [{:keys [status data]} preview
...
(defoverride ^:defer message-link-preview [msg-type-tag raw-body room-id]
